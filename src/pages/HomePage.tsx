...
const TOPICS = [
  {
    ...
    points: ['So sánh A/B ngay trong 1 app', 'Xem code sau khi compile', 'Khi nào compiler bỏ qua'],
  },
  {
    to: '/use-optimistic',
    ...
    points: ['Chat gửi tin nhắn', 'Nút Like chống spam click', 'Form + useActionState'],
  },
  {
    to: '/ref-as-prop',
    icon: <AimOutlined />,
    ...
    points: ['forwardRef vs ref prop', 'useImperativeHandle', 'Cleanup cho ref callback'],
  },
]
    ...
    <>
      <section className="home-hero">
        ...
        <Typography.Paragraph
          className="dim"
          style={{ fontSize: 18, maxWidth: 760, margin: '0 auto', lineHeight: 1.7 }}
        >
          Ba tính năng thay đổi cách chúng ta viết React hằng ngày. Mỗi tính năng là một trang
          riêng, mỗi ví dụ đều có <b style={{ color: 'var(--text)' }}>code bên trái</b> và{' '}
          <b style={{ color: 'var(--text)' }}>kết quả chạy thật bên phải</b> — bấm được, sửa được,
          không phải ảnh chụp.
        </Typography.Paragraph>
      </section>
        ...
        {TOPICS.map((topic) => (
          <Col key={topic.to} xs={24} lg={8}>
            <Link to={topic.to} className="topic-card">
              ...
              <p className="dim" style={{ margin: '4px 0 10px', lineHeight: 1.65 }}>
                {topic.desc}
              </p>
              <ul className="dim" style={{ margin: 0, paddingLeft: 18, lineHeight: 1.9, fontSize: 14 }}>
                {topic.points.map((p) => (
                  <li key={p}>{p}</li>
                ))}
              </ul>
              <span style={{ marginTop: 'auto', paddingTop: 16, color: 'var(--primary)', fontWeight: 600 }}>

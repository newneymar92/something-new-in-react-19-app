...
const INITIAL: Message[] = [
  { id: 'm1', text: 'Chiều nay demo React 19 nhé?', author: 'them', time: '14:02' },
  { id: 'm2', text: 'Ok, mình chuẩn bị slide rồi 👍', author: 'me', time: '14:03' },
]
...
export default function ChatDemo() {
  ...
  const [optimisticMessages, addOptimisticMessage] = useOptimistic(
    ...
    (current: Message[], text: string): Message[] => [
      ...current,
      { id: `tmp-${Date.now()}`, text, author: 'me', time: nowTime(), pending: true },
    ],
  )
  ...
  return (
    <form action={sendAction}>
      ...
      <Space.Compact style={{ width: '100%', marginTop: 12 }}>
        <Input name="text" placeholder="Nhập tin nhắn rồi Enter..." autoComplete="off" />
        <SendButton />
      </Space.Compact>
    </form>
  )
...
function SendButton() {
  ...
  return (
    <Button type="primary" htmlType="submit" icon={<SendOutlined />} loading={pending}>
      Gửi
    </Button>
  )
}
  ...
  return (
    <>
      <Space size={8} style={{ marginBottom: 8 }} wrap>
        <Tag className="mono">state thật: {messages.filter((m) => !m.pending).length} tin</Tag>

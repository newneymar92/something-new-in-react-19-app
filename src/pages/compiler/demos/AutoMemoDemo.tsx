      ...
      <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
        {members.map((m) => (
          <Space key={m.id} size={8}>
            <Avatar
              size={26}
              style={{ background: m.color, color: "#0a0c11", fontWeight: 700 }}
            >
              {m.name.at(0)}
            </Avatar>
            <span>{m.name}</span>
...
export default function AutoMemoDemo() {
  ...
  return (
    <div>
      ...
      >
        Bấm nút vài lần: cả hai <code>Parent</code> đều render lại (vì prop{" "}
        <code>count</code> đổi), nhưng chỉ bên trái là <code>TeamList</code>{" "}
        render theo. Bên phải compiler đã cache sẵn phần tử JSX{" "}
        <code>&lt;TeamList members=&#123;members&#125; /&gt;</code> nên React bỏ
        qua luôn.
      </p>
    </div>
  );
